...
model_path = "./results/trained_models/meomeo.hdf5"
    ...
    if ret:
        ...
        if results.multi_hand_landmarks:
            ...
            for hand, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                cv2.rectangle(frame_fliped, (box_rect[0], box_rect[1]), (box_rect[2], box_rect[3]),
                              (0, 0, 0), 1)

                processed_landmark_list = np.expand_dims(np.array(processed_landmark_list), axis=0)

                dae = model.predict(processed_landmark_list, verbose=0)
                sign = gestures[np.squeeze(np.argmax(dae))]
                frame_fliped = draw_info_text(image=frame_fliped, brect=box_rect, handedness=handedness, hand_sign_text=sign)
